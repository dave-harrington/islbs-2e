...
Bernoulli random variables arise in many settings.  Suppose an individual joins the hypothetical HMO described in @sec-cost-of-hmo-insurance and their demographic characteristics match the population used to construct the probability distribution of number of visits in a year.  If $X$ is the variable that takes on the value 1 if the new member has one or more outpatient visits and 0 if no visits, then $X$ will be a Bernoulli variable with probability of success $p$ = 1 - `r v_probs[1]` = `r 1 - v_probs[1]`.  Suppose 10 new members from this demographic joined the plan during the last year, and whether they had 1 or more outpatient visits was recorded in the sequence of ones and zeros: {0, 1, 1, 1, 1, 0, 1, 1, 0, 0}. The observation on each new member can be regarded as a Bernoulli trial, and the observed sequence as 6 successes and 4 failures. 
...
## The geometric distribution {#sec-geometric-dist}
...
When the mean and standard deviation are not specified, qnorm assumes a standard normal and  returns the $Z$-score for that percentile.
...
When events accumulate over time in such a way that the probability an event occurs in an interval is proportional to the length of an interval and that the number of events in non-overlapping intervals are independent, the parameter $\lambda$  represents the average number of events per unit time, i.e., the rate per unit time.
...
The histogram in @fig-ami-incidences-over-100-days  approximates a Poisson distribution with rate equal to 4.4 events per day, for a population of 8 million. 
...
In New York City, what is the probability that (a) at most 2 individuals are hospitalized for AMI in seven days, (b) at least 3 individuals are hospitalized for AMI in seven days?  [^important-distributions-401]
:::

[^important-distributions-401]: For (a), 
\begin{align*} 
P(X \leq 2) &= P(X=0) + P(X=1) + P(X=2) \\
&= \frac{e^{-4.4 \times 7}(4.4 \times 7)^{0}}{0!} + \frac{e^{-4.4 \times 7}(4.4 \times 7)^{1}}{1!} + \frac{e^{-4.4 \times 7}(4.4 \times 7)^{2}}{2!} 
&= 2.12 \times 10^{-11}
\end{align*}
(b) $P(X \geq 3) = 1 - P(X < 3) = 1 - P(X \leq 2) = 1 - 2.12 \times 10^{-11} \approx 1 $
...
## The negative binomial distribution {#sec-neg-binomial-dist}

The geometric distribution in @sec-geometric-dist describes the probability of observing the first success on the $k^{th}$ trial. The \index{negative binomial distribution} **negative binomial-distribution**  is more general: it describes the probability of observing the $r^{th}$ success on the $k^{th}$ trial.

Suppose a research assistant needs to successfully extract RNA from four plant samples before leaving the lab for the day. Yesterday, it took 6 attempts to attain the fourth successful extraction. The last extraction must have been a success; that leaves three successful extractions and two unsuccessful ones that make up the first five attempts. There are ten possible sequences, which are shown in @tbl-rna-extractions.

$$
\newcommand{\succObs}[1]{\color{blue}{\stackrel{#1}{S}}}
$$
...
: The ten possible sequences when the fourth successful extraction is on the sixth attempt. {#tbl-rna-extractions}

::: {.guidedpractice data-latex=""}

Each sequence in @tbl-rna-extractions has exactly two failures and four successes with the last attempt always being a success. If the probability of a success is $p=0.8$, find the probability of the first sequence. [^important-distributions-402]
:::

[^important-distributions-402]: The first sequence: $0.2\times0.2\times0.8\times0.8\times0.8\times0.8 = 0.0164$.

If the probability of a successful extraction is $p=0.8$, what is the probability that it takes exactly six attempts to reach the fourth successful extraction? Using the above example, there are 10 different ways this event can occur. The probability of the first sequence is 0.0164, and each of the other sequences have the same probability. Thus, the total probability is $(10)(0.0164) = 0.164$.
...
$$
{k-1 \choose r-1} = \frac{(k-1)!}{(r-1)! \left((k-1) - (r-1)\right)!} = \frac{(k-1)!}{(r-1)! \left(k - r\right)!}.
$$
...
::: {.important data-latex=""}
**Four conditions to check before using the negative binomial model**
...
\begin{align*}
P(4^{th} \text{ success on } 5^{th} \text{ trial }) &= {k-1 \choose r-1}p^r(1-p)^{k-r} \\
...
Assume that each extraction attempt is independent. What is the probability that the fourth success occurs within 5 attempts?  [^important-distributions-403]
:::

[^important-distributions-403]: If the fourth success ($r=4$) is within five attempts, it either took four or five tries ($k=4$ or $k=5$):
\begin{align*}
...
Suppose that out of 9 deer, 4 are marked. What is the probability of observing 1 marked deer in a sample of size 3, if the deer are sampled without replacement? First, consider the total number of ways to draw 3 deer from the population; As shown in @fig-h-geom-schematic, samples may consist of 3, 2, 1, or 0 marked deer. There are ${4 \choose 3}$ ways to obtain a sample consisting of 3 marked deer out of the 4 total marked deer. By independence, there are ${4 \choose 2} {5 \choose 1}$ ways to obtain a sample consisting of exactly 2 marked deer and 1 unmarked deer. In total, there are 84 possible combinations; this quantity is equivalent to ${9 \choose 3}$. Only ${4 \choose 1} {5 \choose 2} = 40$ of those combinations represent the desired event of exactly 1 marked deer. Thus, the probability of observing 1 marked deer in a sample of size 3, under sampling without replacement, equals $40/84 = 0.476$.

```{r}
#| label: fig-h-geom-schematic
#| out-width: 90%
#| fig-cap: |
#|   Possible samples of marked and unmarked deer in a sample $n = 3$, where $m = 4$ and $N - m = 5.$ Striped circles represent marked deer, and empty circles represent unmarked deer.
#| fig-alt: |
#|  The figure shows the outcome of observing 1 marked deer in a sample of size 3 from a population of 9 deer, of which 4 are marked.  A box on the left side of the figure shows the 5 unmarked and 4 marked deer as 9 discs, 5 empty and 4 shaded.  The next figure shows all the possible outcomes of sampling 3 deer in a 3 x 4 matrix of discs.  In the top row all three are shaded, indicating three  marked deer chosen.  Each of the subsequent rows shows, respectively, 2, 1 and 0 marked deer chosen. The last panel in the plot shows the sum of the binomial coefficients corresponding to each of the 4 possible numbers of deer chosen, and inidates that the sum in 84.
#| fig-pos: H
knitr::include_graphics("images/h-geom-schematic.png")
```

::: {.guidedpractice data-latex=""}

Suppose that out of 9 deer, 4 are marked. What is the probability of observing 1 marked deer in a sample of size 3, if the deer are sampled with replacement?  [^important-distributions-405]
:::

[^important-distributions-405]:  Let $X$ represent the number of marked deer in the sample of size 3. If the deer are sampled with replacement, $X \sim \textrm{Bin}(3, 4/9)$, and $P(X = 1) = { 3 \choose 1 } (4/9)^1 (5/9)^2 = 0.412$.
...
$$ {#eq-hypergeometric-prob}

Let $p = m/N$, the probability of success. The mean and variance are given 
\begin{align*}
\mu &= np \\
...
::: {.important data-latex=""}
**Is it hypergeometric? Conditions to check.**

1.  The trials are dependent. 

1. Each trial outcome can be classified as a success or failure. 

1.  The probability of a success is different for each trial.

:::

::: {.guidedpractice data-latex=""}

A small clinic would like to draw a random sample of 10 individuals from their patient list of 120, of which 30 patients are smokers. (a) What is the probability of 6 individuals in the sample being smokers? (b) What is the probability that at least 2 individuals in the sample smoke? [^important-distributions-406]
:::

[^important-distributions-406]: \(a\) Let $X$ represent the number of smokers in the sample. $P(X = 6) = \dfrac{{30 \choose 6}{90 \choose 4}}{{120 \choose 10}} = 0.013$. (b) $P(X \geq 2) = 1 - P(X \leq 1) = 1 - P(X = 0) - P(X = 1) = 1 - \dfrac{{30 \choose 0}{90 \choose 10}}{{120 \choose 10}} - \dfrac{{30 \choose 1}{90 \choose 9}}{{120 \choose 10}} = 0.768$. 
