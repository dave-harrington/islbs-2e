...
A random variable assigns numerical values to the outcome of a random phenomenon, and is usually written with a capital letter such as $X$, $Y$, or $Z$. Random variables are characterized as either discrete or continuous.

### Discrete random variables {#sec-distributions-rv}
...
Discrete distributions take on a finite number of possible outcomes.  But some measurements can have all values in an interval, such as height or weight and they are best modeled with continuous random variables.  The distribution of a continuous random variable is specified by \index{probability density function} **probability density function** instead of a table.

Figure~\ref{fdicHeightContDist} shows a histogram of the height for 3 million US adults from the mid-1990's, with an overlaid density curve. ^[This sample can be considered a simple random sample from the US population. It relies on the USDA Food Commodity Intake Database. 
