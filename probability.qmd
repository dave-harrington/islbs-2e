...
This definition relies on a result from probability theory called the \index{Law of Large Numbers} **Law of Large Numbers**, which states that when a random phenomenon is repeated may times the proportion $\hat{p}_n$ of occurrences of a particular outcome converges to the probability $p$ of that outcome.  In simple settings like rolling a six-sided die, where probabilities are calculated by enumerating a sample space, the probability of an outcome such as rolling a 1 will match the proportion that a 1 is observed after many rolls.  
...
The  notation of set theory is used to describe events.  The complement $A^c$ of an event $A$ consists of all the outcomes in $S$ not in $A$.  Two events $A$ and $B$ are **disjoint** \index{disjoint events} or **mutually exclusive** \index{mutually exclusive}  if they have no outcomes in common.  The event $(A \text{ and } B)$ consists of the outcomes in $S$ that are members of both $A$ and $B$, sometimes denoted as members that satisfy both $A$ and $B$.  The event $(A \text{ or } B)$ consists of the outcomes that are either in $A$ or $B$ or possibly both.  In other texts, the event $(A \text{ and } B)$ is sometimes written with the set theory notation for intersection, $A \cap B$, or simply as $A,B$, and the event $A \text{ or } B$ uses the notation for union, $A \cup B$.  For conciseness, sentences such as "$B$ is the event that a child has CF" are written as $B$ = {child has CF}. 
...
There are 6 possible outcomes for the first die, and for each of those outcomes there are 6 possible outcomes for the second die, so the sample space consists of the 36 ordered pairs $(j,k)$, where $j$ and $k$ each can be any of the integers between 1 and 6.
:::
...
Since $P(S)$ = 1, $P(S^c)$ = 0, so any event which does not include any outcomes has probability 0.  As a consequence, if $A$ and $B$ are disjoint events, $P{A \text{ and } B}$ = 0.
...
A little less than 50% of this cohort rated their health as other than good.
    ...
    As always, it pays to be careful with the language used in a problem statement.  The phrase **other than good**  makes it clear that it includes excellent health; the phrase **not good** (for instance) might be interpreted as any outcome worse than good.
...
Suppose  $B$ is the event that a randomly selected member of the US adult population has been diagnosed with type 2 diabetes.  According to the CDC data, $P(B)$ =  0.113, or 11.3%. @tbl-age-diabetes-prevalence shows that if one adds the information that the adult is between 20 and 39, that probability decreases to 0.022. The probability  has changed because, instead of sampling from the sample space of the entire adult population, the random selection is from a reduced sample space, adults between 20 and 39 years old. In the language of probability, 0.022 is the \index{conditional probability} **conditional probability** that an individual has been diagnosed with diabetes, given that (or conditioning on) the person between 20 and 39 years old.   
...
(a) @tbl-sum-two-dice shows that if the sum is 4, the outcome of the roll must have been (1, 3), (2, 2), or (3, 1). In this reduced sample space of 3 possible outcomes, each of the three are equally likely so each of will have occurred occurred with probability 1/3, so $P(B|A) = 1/3$. 
...
Conditional probabilities can be calculated from tables such as @tbl-gss-age-health.  Let $A$ = {age 26 to 45}.  Conditioning on the event $A$ = {age 26 to 45} means calculating probabilities for reported health among the `r tbl_age_health[2,5]` respondents satisfying in that age group.  Conditional probabilities for a reported health rating will be the proportion of the observed outcomes among the number of respondents between 26 and 46. Conditioning on any other age group involves calculating proportions for outcomes in that age group.  These conditional probabilities  will be the row proportions in a table like @tbl-gss-age-health, but with counts replaced by row proportions.  The  modified table is shown in @tbl-gss-age-health-row-prop.
...
The first step is write what is known and what is asked in probability notation.  Let $A$ = {randomly selected person is alive at age 80 or older}, and $F$ = {randomly selected person is a females}.  In the problem statement $P(F \text{ and } A)$ = 0.074, $P(F^c \text{ and } A)$ = 0.044, and $P(F)$ = 0.505. Part (a) asks for the Venn diagram showing the relationship between $A$ and $W$. Parts (b) and (c) ask for $P(A \mid F)$ and $P(F \mid A)$, respectively, while (d) asks for an explanation of the conditional probabilities that does not use the jargon of probability.
...
#| out-width: 80%
#| fig-cap: | 
#|     Venn diagram showing the relationship between A and W, without probabilities
library(VennDiagram)
...
@fig-female-80plus-venn adds the probabilities to the diagram.  The probability of selecting a woman is known, so $P(F)=$ 0.505, as is the size of the intersection, $P(F \text{ and } A)$ = 0.074. Since the Venn diagram shows areas that do not include overlap, the probability shown in left side of the blue oval is 0.505 - 0.074 = 0.431.  The size of the oval on the right that does not include the overlap, $P(F^c \text{ and } A )$, is the probability that someone is male and 80+, given as 0.044.   
...
#| out-width: 80%
#| fig-cap: | 
#|     Venn diagram showing the relationship between A and W, with probabilities
library(VennDiagram)
# Draw Venn diagram with specified areas
age_venn <-  draw.pairwise.venn(
  area1 = 0.505,     # set A
...
(d)  Part (b): Approximately 15% of the females in the US are at least 80 years old.  Part (c): Among the elders in the US who are at least 80 years old, approximately 63% are female.  So a majority of people in the US at least 80 years old are female.  Women do live longer than men.
