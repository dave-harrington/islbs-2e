...
a) Suppose that once we know an employee’s true drug-use status, knowing the result of one test gives us no additional information about how the employee will perform on the other test. Under that assumption, what is the probability that a drug-free employee tests positive on both tests?
