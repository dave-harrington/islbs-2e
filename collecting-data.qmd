...
- Add example of mask adoption studies: ipsos vs facebook

- Exercises using data dashboard in CDC
...
Much of the important work in using data to study issues in public health and medicine happens before data collection and involves answers to the following questions:

* What question does the study team hope to answer?  The question should be as specific as possible.  Rather than the important but vague "What is the quality of post-natal care in rural Maine?", a better question is "In the five counties in Maine with the lowest population density, what proportion of infants die in the first year of life?".

* What is the best way to collect data to answer that question?  In statistical terms, how should the study be designed?

* What are the sources of error in data collection that should be controlled, at least as much as possible?

* Are there important questions the study will not be able to answer?  If so, perhaps the design should be reconsidered.

This chapter discusses three main types of studies:  randomized experiments, observational studies, and sampling from a population.  Each has important advantages and constraints, but done well, studies of each type lead to progress in treating disease or in understanding the health of a population.
:::
...
At age five, each child underwent an oral food challenge (OFC) test to assess peanut allergy. The test involved consuming 5g of peanut protein. A PASS indicated no allergic reaction, while a FAIL indicated an allergic reaction.

The primary analysis used data from 530 children with a negative skin test at the study's start. However, data were unavailable for 12 children.

The LEAP [data](http://openintrostat.github.io/openintro/reference/LEAP.html) data are available in the [**openintro**](http://openintrostat.github.io/openintro) R package. Individual-level data from the study are shown in @tbl-leap-study-results-df, which includes 6 children with a negative skin prick test at randomization. Each row represents a participant, showing their `participant.ID`, `treatment.group`, and `overall.V60.outcome` (OFC result at 60 months). 
...
The table shows patterns in the data. The intervention failed to prevent an allergy in 15.6% of children, but the proportions differ: 13.7% in the avoidance group and 1.9% in the consumption group.

The difference in OFC failure proportions is 11.8%, or nearly 12% more in the avoidance group. The ratio of the two proportions is 0.137/0.019 = 7.31, indicating the avoidance group has a 7 times higher risk of a subsequent peanut allergy.

Even without advanced methods, the LEAP study results are striking. Early peanut product exposure seems effective in reducing the likelihood of later peanut allergies. Is it reasonable to assume peanut product consumption caused this reduction? 
...
This study illustrates important issues in \index{evidence-based medicine} **evidence-based medicine**. At the time, avoiding peanut products was the prevailing approach to preventing peanut allergies in at-risk children. LEAP  challenged this belief, providing evidence that avoidance was not the most effective strategy. In 2017 the National Institute of Allergy and Infectious Diseases issued [new guidelines](https://www.niaid.nih.gov/sites/default/files/addendum-peanut-allergy-prevention-guidelines.pdf) for the management of children susceptible to peanut allergies based largely on the LEAP findings.

Importantly, the study required informed consent from parents to randomly assign their children to be randomly assigned to one of two interventions with uncertain outcomes. In return investigators ensured that the study was justified by preliminary data, had a sound design, and protected the well-being of the participants. ^[The 2024 update to the World Medical Association Ethical Principles for Medical Research Involving Human Participants (also known as the Declaration of Helsinki) can be found in 
@wma2024jama]
...
4. **Blocking** Experiments use \index{blocking} **blocking** when a few variables may significantly influence the response. Participants are grouped into blocks based on attributes and randomized to treatment within each block. This is also called \index{stratifying} **stratifying**. The LEAP study team stratified infants into two cohorts based on the skin test outcome: red, swollen mark (wheal). Infants were randomized between peanut consumption and avoidance groups within each block. 
...
A gardener wants to test two fertilizers for rose bushes on her 12 $\times$ 12 meter plot. She plans to organize the garden in a checkerboard pattern of 16 3 $\times$ 3 meter squares, with two square plots with and without bushes in each row.

Design an experiment to evaluate the fertilizers. Specify a question to investigate and a response variable.
...
A large hospital plans to test a new method of insuring its physician assistants (PAs) accurately complete vital signs and symptom measurements at its outpatient clinic. Currently, PAs attend a training session for one of four specialties, see patients after passing an exam, and use a specialized checklist. The hospital considers eliminating the training session and equipping PAs with a standardized electronic checklist on tablets.


Each June, the organization hires 80 new PAs assigned to their program of choice.  Typically at least 16 elect each of the practices.

Describe how the principles experimental design might be used in an experiment to evaluate the new checklist. Specify a response variable and how it will be measured.
...
These principles have revolutionized the practice medicine and shaped public policy. The Salk vaccine trials (1954-1955) demonstrated effectiveness in preventing polio, based on results from 401,974 randomized children. The long-term impact has been significant. Our World in Data's @fig-polio-vaccine-owid shows the substantial decrease in polio-related paralysis between 1980 and 2020 after global vaccine adoption.
...
More recently, a study of 42,548 participants established the efficacy of the BNT162b2 mRNA Covid-19 vaccine [@polack2020safety]. Small studies can be effective as well. A study of 42,548 participants [@polack2020safety] established the efficacy of the BNT162b2 mRNA Covid-19 vaccine. Small studies can be effective too. A 1994 study [@connor1994hiv] of 477 women showed that AZT reduced the rate of mother-to-child transmission when HIV-infected women gave birth.
...
LEAP was not conducted in a vacuum; it wouldn't have been ethical to randomize infants prone to allergies to a diet of peanut consumption without prior data suggesting the strategy might be effective.  LEAP was preceded by several studies, including one in which investigators compared diets and the presence of peanut allergies in the United Kingdom (UK) and Israel [@du2008early]. The study  found that 1.85% of UK children had peanut allergies compared to 0.17% in Israel. However, Israeli infants consumed 7.1 g of peanut protein per month, compared to 0 g in the UK. A strong negative association between peanut consumption and peanut allergies was observed.
...
Observational studies can be based on either newly collected or existing data.  The 2008  study used questionnaires to collect data on diet and peanut allergies in UK and Israeli school children.  The case study in @sec-developmental-disability examining the association between expenditures and race of a consumer was an observational study using  existing California DDS data.  

### Guidelines for designing observational studies {#sec-observational-studies-guidelines}

As in randomized experiments, observational studies should be based on a specific question about a response and potential explanatory variables.  Guidelines for the design of observational studies are similar to the priniciples of experimental design, but with some important modifications.
...
How does the opinion about sidewalk conditions in a city held by citizens with disabilities  differ from those without disabilities?   @fig-somerstat-2023-disability-sidewalks shows the association between disability status and attitudes about sidewalk maintenance in Somerville MA, suburb of Boston.  Citizens with a disability are more likely to report being either unsatisfied or very unsatisfied with sidewalk maintenance than those without a disability. Perhaps surprisingly, the proportion of residents who are very satisfied with sidewalk maintenance is about the same in both groups.
...
Many important features of a population are characterized by **parameters** \index{parameters of a distribution}, such as the average amount of carbon sequestered in the Portland park trees.  The mean carbon stored in the sample of 500 was a **statistic**, an estimate of the population average based on that sample.  The sample of 500 trees should be representative of the population because trees in the sample were selected randomly.   

The since the R package [`pdxTrees`](https://github.com/mcconvil/pdxTrees) contains data on all park trees, it represents a rare instance in which population parameters can be directly compared with their sample-based estimates.  

Just as random treatment allocation is used to prevent bias in experiments, random sampling is used prevent bias in surveys. This section discusses four random sampling methods: simple random, systematic sampling, stratified sampling and multistage cluster sampling.
...
Random sampling is the best way to ensure that a sample reflects a population. In a \index{simple random sample}{sample!simple random sample} **simple random sample (SRS)**, each member of a population has the same chance of being selected. One way to achieve a simple random sample of the health plan members is to randomly select a certain number of names from the complete membership roster, and contact those individuals for an interview.  This process is illustrated schematically in the upper panel of @fig-sample-random-health-plan.
...
In \index{stratified sampling}{sample!stratified sampling} **stratified sampling**, the population is first divided into groups called \index{strata}\index{sample!strata|textbf} **strata** before cases are selected within each stratum (typically through simple random sampling).  Stratified sampling is illustrated in the lower panel of @fig-simple-stratified. Strata are chosen such that similar cases are grouped together. Stratified sampling is especially useful when the cases in each stratum are very similar with respect to the outcome of interest, but cases between strata might be quite different. 

Suppose that the health care organization has facilities in different cities and wants to study some measure of quality of care. If the range of services offered differ by city, but all locations in a given city offer similar services,  the quality improvement team might use stratified sampling to identify participants for their study, where each city represents a stratum and plan members are randomly sampled from each city.
...
**Response bias** \index{response bias}  Add explanation
