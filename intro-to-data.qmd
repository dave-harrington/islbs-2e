...
Data from the [`Portland Oregon Tree Inventory project`](https://www.portland.gov/trees/get-involved/treeinventory)can be used to study that question. The project has compiled a comprehensive inventory of trees in Portland, Oregon, and is used with other data in maintenance planning for the Portland urban forest. The urban forest is a particularly valuable resource.  According to [`Portland’s city government`](https://www.portland.gov/council/documents/ordinance/passed/191975), trees provide an estimated $\$40$ an million annually in environmental benefits to Portland, including air pollution removal, avoided storm water runoff, and carbon sequestration. 
...
- "Good": Tree is an excellent representative for it’s species. Roots are sound, trunk is solid with no
...
In the United States, individuals with developmental disabilities typically receive services and support from state governments. In California, funds are allocated to developmentally-disabled residents through the California Department of Developmental Services (DDS). Individuals receiving DDS funds are referred to as ‘consumers’. California law specifies that no individual may be denied funds or services based on their race or ethnicity.


In the early 2000s, a team of researchers examined the mean annual expenditures on consumers by ethnicity. They found that the mean annual expenditures for Hispanic consumers were approximately one-third of the mean expenditures for White not Hispanic consumers. This finding led to allegations of ethnic discrimination against the California DDS.


Does this disparity suggest ethnic discrimination, or might there be additional factors influencing the relationship? This case study explores the relationship between the response variable, `expenditures`, and the explanatory variable, `ethnicity`, while investigating whether other variables may contribute to the observed differences.
...
*I could not find a reference to the specific claim of discrimination*
...
: Variable descriptions for `dds.discr`. {#tbl-dds-discr-variables} {tbl-colwidths="[25,75]"}
...
Even when the primary intent of an analysis is to explore relationships in the data, it is important to first understand the characteristics of the variables by examining their distributions. In this dataset, key questions to guide the exploration include: *What is the range of annual expenditures?*, *Do consumers tend to be older or younger?*, and *Are there more consumers from one ethnic group than another?*


**Distributions of the variables**

@tbl-dds-discr-exp-five-num-summ and @fig-dds-discr-boxplot-histogram show the five number summary (plus the mean) and the right-skewed distribution of`expenditures`. As expected, the mean expenditures is larger than the median.  Even though mean `expenditures` is `r m_r`,  the majority of consumers have expenditures less than approximately the median, `r med`.  The mode at the left most bar in the histogram indicates that the largest number of expenditures are less than \$5,000.  The box plot shows that despite the right skewing, there are no outliers in the data set, but it does show at least one large observation that is not evident in the histogram --- the maximum expenditure `r max_r`(USD).

For the most consumers, expenditures are relatively low,  between \$0 and \$5,000.   The middle 50% of expenditures are between the first ($Q_1$) and third quartiles ($Q_3$), `r q1_r` (USD) and `r_q3_r`. There are some consumers with much higher expenditures,  between \$60,000 and \$80,000.


```{r dds-discr-exp-five-num-summ}
#| label: tbl-dds-discr-exp-five-num-summ
#| tbl-cap: |
#|   The five number summary and the mean for expenditures in `dds.discr`.  Va
#| tbl-pos: H
...
#|    - Bar plot of the age categories
#| fig-alt: |
...
The distribution for `expenditures` shown in the histogram is a \index{mixture distribution} **mixture distribution**.  In a mixture distribution two or more subsets of observations appear to follow different underlying distributions.  Expenditures less than \$20,000 occur much more frequently in the range 0 - \$5,000 and become increasingly rare as expenditures increase to \$20,000.  Expenditures centered at approximately $42,000 are symmetrically distributed.

A consumer's age in years is recorded as  `age`; in `age.cohort`, consumers are assigned to one of six age cohorts that match particular life phases. In the first three age cohorts, consumers are still living with their parents as they move through preschool, elementary/middle school, and high school. In the 18 - 21 cohort, consumers are typically transitioning from parental or foster homes to independent supported settings. From ages 22 - 50, individuals are generally no longer live with parents or guardians but may still receive some support from family. In the 51+ cohort, consumers often have no living parents and typically require the most amount of support.


@fig-dds-discr-boxplot-histogram(a) reveals the right-skewing of `age` --- most consumers are younger than 30. @fig-dds-discr-boxplot-histogram(b) shows that here are approximately 200 individuals in each of the middle four cohorts, while there are about 100 individuals in the other two cohorts.

```{r}
#| label: fig-dds-discr-age-hist-barplot
#| fig-cap: |
#|   A histogram and bar plot showing the distributions of `age` and `age.cohort`
#| fig-subcap: |
#|    - Histogram of `age`, recorded in years
#|    - Bar plot showing frequencies in the categoris of `age.cohort`
#| fig-width: 4
#| layout-ncol: 2
...
The proportion of females and male consumers is nearly identical in this sample , as is shown in @tbl-dds-discr-gender-relative-frequency.

```{r dds-discr-gender-rel-freq-tbl}
#| label: tbl-dds-discr-gender-relative-frequency
...
```{r dds-discr_ethnicity-barplot}
#| label: fig-dds-discr-ethnicity-barplot
#| fig-cap: |
...
b <- (df$ethnicity) == "White not Hispanic"
m_hisp <- round(mean(df$expenditures[a]), 0)
m_not_hisp <- round(mean(df$expenditures[b]), 0)
ratio_eth <- round(100 * m_hisp/m_not_hisp, 2)
```

Do the distributions of `expenditures` across ethnic groups show the possibility of ethnic bias, as in the original claim? @tbl-dds-expenditures-by-ethnicity-summ  shows the mean and median expenditures rounded to the nearest dollar, along with the number of observations by `ethnicity`. Mean `expenditures` vary widely across groups, but, consistent with the bar plots in @fig-dds-discr-ethnicity-barplot, three groups (`American Indian`, `Native Hawaiian` and `Other`) have too few observations to be informative.  Among the groups with more than 100 observations the largest discrepancy in mean expenditures is between `Hispanic` and `White not Hispanic`, `r m_hisp` versus `r m_not_hisp`.  Hispanic expenditures were `r ratio_eth`% of those for Whites, not Hispanic, suggesting the possibility of bias in funding.   This analysis focuses on those two groups.



```{r tbl-dds-expenditures-by-ethnicity-summ}
#| label: tbl-dds-expenditures-by-ethnicity-summ
#| tbl-cap:  "Mean and median expenditures (USD) and number of cases by ethnicity, `dds.dscr`"
#| alt-cap:  | 
...
```{r}
#| label: dds-exp-ethicity-box-plot
#| fig-cap: |
#|       Side by side box plots of expenditures, Hispanic and White not Hispanic
#|          ethnic groups
#| alt-cap: |
#|       Coming
#| fig-width: 8
...
```{r}
#| label: dds-ethicity-age-rel-freq-bar
#| fig-cap: |
#|       Relative frequency bar plot of ethnicity by age cohort.
#| alt-cap: |
#|       Coming
#| fig-width: 8
