...
<!--
Sequencing the dds presentation

- Background, a bit more about the disabilities
- Joint dist eth/exp
  - Table: remove counts

- Distributions of response, explantory variables
    - Eth box, numbers at the top
    - Finsh with age distribution
    - State that analysis will proceed with only White, White non-Hispanic

- Relationships between response and explanatory variables
    - Ethnicity/exp box plots (two groups)
    - Sex/exp table or box
    - Age/exp scatter (add color to scatter), box by category
    
- Relationships among the explanatory variables 
    - Argument for looking at age/eth  Age distribution by eth Ca
        - Rel freq bar, dogged bar, numbers at the top
    - Rel freq table age/eth
    
- Relationship between eth/exp, accounting for age


- Simpson's paradox
    - Simple example, perhaps worked example
...
Among groups with more than 100 observations, the largest discrepancy is between Hispanic and White non-Hispanic, with mean expenditures of `r m_hisp` and `r m_not_hisp`, respectively. Hispanic expenditures were `r ratio_eth`% of those for White non-Hispanics, suggesting the possibility of bias in funding.
...
```
Does this disparity suggest ethnic discrimination, or might additional factors influence the relationship? This case study examines the relationship between the response variable, `expenditures`, and the explanatory variable, `ethnicity`, while investigating whether the additional explanatory variables `age`, `age.cohort` and `gender` may contribute to the observed differences in funding across ethnic groups. Like many analyses, this one is guided by both interesting features in the data and hypotheses about the interrelationships among the variables.
...
@fig-dds-exp-ethnicity-boxplot shows the distribution of `expenditures` in `Hispanics` and `White non-Hispanics`. It is consistent with the summary statistics in the earlier @tbl-dds-exp-ethnicity-summ.  Median expenditure in Hispanics (`r med_hisp`(USD) is much lower than in White non-Hispanics(`r med_not_hisp `(USD)). 
...
If the Hispanic population in California is younger than the White non-Hispanic population, the lower expenditures in Hispanics may be influenced by lower expenditures by age in @tbl-dds-exp-age-summ rather than ethnicity. The 2007 [California Current Population Survey Report](https://dof.ca.gov/wp-content/uploads/sites/352/Reports/Demographic_Reports/documents/CACPS07_final.pdf)
states that "The median age for all race/ethnic groups in California was 34, ranging from 41 for Whites to 27 for Hispanics".  @fig-dds-eth-age-barplot shows a similar relationship between age and ethnicity in the `dds.discr` data.
