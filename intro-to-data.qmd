...
This section explores some of the variables Brockmann's team recorded 173 female-male mating pairs, described in more detail in Table 3 of her article. The dataset includes 5 recorded measurements for each mating pair and is available on Alan Agresti's University of Florida [`web page`](https://users.stat.ufl.edu/~aa/cda/data.html). This section introduces the variables in the data. @sec-relationships-one-numerical-one-categorical examines how some features of the female in a mating pair tend to attract unattached males.  The five variables in the data used here are defined in @tbl-crabs-variable-descriptions.
...
The variables `color` and `spine` are \index{categorical variables} **categorical variables**, also called \index{qualitative variables} **qualitative variables**. The values of categorical variables are names or labels; in this case the values are carapace colors or the condition of the spines of the crab. Arithmetic operations cannot be performed on categorical variables, but they can be tabulated. When the labels of a categorical variable have no natural ordering, it is referred to as a **nominal categorical variable**. Both `color` and `pine` is an **ordered categorical variable**, since color has a natural ordering from "light medium" to "dark" and the values of `spine` indicate worsening conditions of the female's spines. In R, categorical variables are also called \index{factor variable} **factor variables**.
...
During photosynthesis, trees remove carbon dioxide from the air, combine it with water, and convert it to sugars and oxygen. [^intro-to-data-3] The sugars are then distributed throughout the tree, and the carbon in them is stored in a process called carbon sequestration. Forests typically store twice as much carbon as they emit and are an important component in mitigating the effect of greenhouse gasses on climate change. How much carbon do trees store?

[^intro-to-data-3]: The sugars created during photosynthesis are primarily sucrose, glucose and fructose.
...
*axes not perfectly aligned, but had trouble with patchwork and cowplot*
...
*return to this.  The discussion is unfinished, rambles, is filled with speculation, and does not properly account for the very different group sizes  Read the Portland documentation for findings and maintenance planning.  Some of this can go in the intro*
...
In the Portland tree data, what is the relationship between the condition of a tree and the amount of carbon it stores?

`Condition` is an ordered categorical variable with "Dead", "Poor", "Fair" and "Good", with the following definitions in the [`training manual`](https://www.portland.gov/sites/default/files/2020/training-book-2019-web-version.pdf) for the inventory project:
...
The \index{side-by-side box plots} **side-by-side box plots**  in @fig-trees-condition-carbon-store-boxplot show how the carbon stored varies with tree condition. There are 6 dead trees in this sample of `r l` but they are included in the box plots since they are storing a mean and median of `r mean_carbon_dt`lbs and `r med_carbon_dt`lbs, respectively.
...
The plot shows (unsurprisingly) that trees in better condition ("Fair" or "Good") store more carbon than trees in poorer condition.  However, even trees in poor condition may have some value as carbon storage organisms.  Several of those trees are storing more than 2000lbs of carbon, with 1 storing nearly 4000lbs. The plot shows why summary statistics without a graph can be misleading; the median carbon stored by the trees in poor condition is approximately the same as those in good condition.

Since 86% of the trees are in "Fair" condition, the distribution of the carbon stored in those trees naturally reflects the distribution in the full sample of `r l` trees shown in @fig-trees-carbon-beeswarm-boxplot -- a skewed distribution with several large outliers.  The details in graphs often give rise to important questions.  What are the features of trees in fair condition that store large amount of carbon?  Does the have implications for forest management if resources become limited?

Perhaps the trees in fair condition are older than those in good condition -- they have had more time to accumulate carbon.^[Tree age was not estimated in the inventory project]  What about the rate at which carbon is being sequestered? How does it vary with tree condition?  `Carbon_Sequestration_lb` is an estimate of the annual rate at which carbon is being stored. The box plots in @fig-trees-condition-carbon-seq-boxplot how that varies with condition.  Trees in good condition have the same median sequestration rate as those in fair condition.
...
*put back infant mortality, show that transformations do not help*
