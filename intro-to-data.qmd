...
How does the amount of stored carbon (`Carbon_Storage_lb`) and the rate at which carbon is sequestered (`Carbon_Sequestration_lb`) vary by tree condition?  In statistical terms, what is the relationship between the explanatory variable tree condition and the response variable carbonn storage?
...
@tbl-trees-condition-frequency shows that the large majority (more than 86%) of the trees are in "Fair" condition.  While there are a few dead trees in the complete inventory, there were none in this random sample.
...
The \index{side-by-side box plots} **side-by-side box plots**  in @fig-trees-condition-carbon-store-boxplot show how carbon storage varies with tree condition. 

@fig-trees-condition-carbon-store-violin offers a different view with  \index{side-by-side violin plots} **side-by-side violin plots** .  Each violin plot is a hybrid of a box plot and density plot. The simplicity of a box plot makes it easy to identify quartiles and outliers, but hides detail on the data density of the response. Violin plots add this information by making the "violin" wider at values of the response where there are more observations.  The red dots in the violin plots denote the median carbon storage at each level tree condition, and the vertical bars extend from the first to the third quantile.
...
<!--
For `satell` versus `width` or `weight` the two correlation coefficients are `r r_width_satell` and `r r_weight_satell`, respectively.  These two relationship are weaker than the `weight` versus `width` relationship, as reflected by the larger point clouds in @fig-crabs-satell-width-weight-scatter that show more variability around the trend line and the band of 0 values along the horizontal axis. 
...
This particular example of a confounder illustrates \index{Simpson's Paradox} **Simpson's Paradox**, a setting in which an association observed in each of several groups may disappear or reverse direction once the groups are combined. Statistical terms, an association between two variables $X$ and $Y$ may disappear or reverse direction once data are partitioned into sub-populations based on a third variable $Z$. 

The clinic billing example also  illustrates the difference between a observed association and a causal relationship.  Based on the association between clinic and average billing, the auditor mistakenly concluded that clinic staff were the cause of the differential billing.  Instead, the different age distributions at the two clinics was the cause.
...
The role of the confounding variable age cohort in the `dds.dscr` is shown graphically in @fig-dds-age-ethnicity-confounding. ^[In the first edition, age cohort was incorrectly labeled a confounder.]
