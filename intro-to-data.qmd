...
<!-- possibly add a stacked bar chart here -->
...
During photosynthesis, trees remove carbon dioxide from the air, mix it with water, and convert it to sugars and oxygen. The sugars are then distributed throughout the tree, and the carbon in those sugars is stored in a process called carbon sequestration. Forests typically store twice as much carbon as they emit, and are one important component of mitigating the effect of greenhouse gasses on climate change. How much carbon do trees store?

The [`Portland Tree Inventory project`](https://www.portland.gov/trees/get-involved/treeinventory) is a comprehensive inventory of trees in Portland, Oregon, compiled to help professionals and citizens protect and grow the  urban Portland forest.  The inventory was compiled separately for trees in neighborhoods and in parks; the park database has $34$ variables for the $25,534$ trees in Portland parks, including such diverse measurements as the age of the tree, its common and scientific name, its financial value, the amount of carbon storage in the tree and is financial value.  This section uses $6$ variables from a random sample of $`r size`$ trees. Tables @tbl-pdx-parks-df and @tbl-pdx-var-definitions show the data for the first 3 and last tree in the data and the variable definitions, respectively. All but the first two variables are numerical variables.
...
#| tbl-cap:  "The first 3 and last of the 500 in the Portland tree data"
#| tbl-pos: H
...
```
 ...
 The numerical variable `Carbon_Storage_lb` is an estimate of the amount of carbon stored in the tree.  What are some of the important features of its distribution?

The distribution of a numerical variable is usually summarized by its center and spread. The \index{mean} **mean**, sometimes called the \indexthis{average}{mean!average}, is a measure of center of a distribution and is denoted (generically) by $\overline{x}$. The average carbon storage in these trees is calculated by summing all the storage values and dividing by the total number of trees, as illustrated below.
...
\begin{align*}
\overline{x} =& \frac{`r x1` + `r x2` + `r x3` \cdots + `r xl`}{`r l`} = `r m`
\textrm{lbs}.
\end{align*}

Since data are almost always a sample from a population, $\overline{x}$ is usually referred to as a \index{sample mean} **sample mean**. These data are a sample from the population of all trees in Portland parks. The \index{population mean} **population mean** of the population from which the sample was selected is denoted by the Greek letter $\mu$.[^ Population means almost never known because data on complete populations is rare.  Since the tree inventory includes all trees in Portland parks, the mean $\mu$ of `Carbon_Storage_lb` can be calculated and is $`r mu`$. ] In the tree data, $\mu$ would be the average of the amount of carbon storage of all $25,534$ trees in the Portland parks.^[ Population means almost never known because data on complete populations is rare.  Since the tree inventory includes all trees in Portland parks, the mean $\mu$ of `Carbon_Storage_lb` can be calculated and is $`r mu`$. ]
...
The \index{median} **median** is another measure of center; it is the middle value of a distribution after the values have been ordered from smallest to largest. The median divides the distribution into two parts; half of the values are less than the median and half are larger. The middle value is well-defined if there are an odd number of observations; it will have the same number of observations below and above it. If the distribution contains an even number of observations, the median is the average of two middle observations. There are $`r size`$ trees in these data; after the carbon storage values have been sorted in ascending order, the median `r M` is the average of the observations in rows $250$ and $251$, $`r middle_low`$ and $`r middle_high`$ respectively. 
...
The standard deviation describes the typical distance between an observation and the mean. The distance of a single observation from the mean is its \index{deviation} **deviation**. The deviations for the $1^{st}$, $2^{nd}$, $3^{rd}$, and $2549^{th}$ observations of carbon storage are: \begin{align*}
x_1-\overline{x} &= `r x1` - `r m` = `r x1 - m` \hspace{5mm}\text{ } \\
x_2-\overline{x} &= `r x2` - `r m` = `r x2 - m` \\
x_3-\overline{x} &= `r x3` - `r m` = `r x3 - m` \\
&\ \vdots \\
x_{2549}-\overline{x} &= `r xl` - `r m` = `r xl - m`.
\end{align*} The units for these $4$ values are lb$^2$.
...
An $\alpha$ **percentile**\index{percentile} is a number with $\alpha$% of the observations below it and $100-\alpha$% of the observations above it. For example, the $90^{th}$ percentile of `Carbon_Storage_lb` ($`r q_9`$) is larger than $90\%$ of the carbon values below and smaller than $10\%$.
:::

The \index{five number summary} **five number summary** is a compact numerical summary providing the largest and smallest observations, the first and third quartiles, and the median. Table @tbl-trees-five-num-summ-carbon shows the five number summary for `Carbon_Storage_lb`, with the mean added for good measure.

```{r five-num-summ}
#| label: tbl-trees-five-num-summ-carbon
#| tbl-cap: |
#|   The five number summary for carbon storage values in Portland tree data.
#| tbl-pos: H
...
The carbon storage values range in size from  $`r min_nr`$ to approximately $`r max_r`$lbs. The median $`r med`$ lbs splits the data in half. Approximately $50\%$ of the observations are between the first and third quartiles, $`r q1_r`$lbs and $`r q3_r`$lbs. The mean $`r m_r`$lbs is considerably larger than the median, and Figure @fig-trees-carbon-beeswarm shows why.

The (aptly named) **beeswarm plot** \index{beeswarm plot} in Figure @fig-trees-carbon-beeswarm shows an interesting feature of the distribution of `Carbon_Storage_lb`.  In regions of the horizontal axis where the plot is wide (many bees are busy) there are a large number of observations. The plot narrows as the number of observations decreases.  There are many trees  storing relatively small amounts of carbon (less than $2,000$lbs), but a few with very large amounts (larger than $4,000$lbs), some larger than $10,000$ lbs.  Averages are sensitive to extreme values at one end of a distribution.  That can easily be seen in the small dataset $2, 4, 6, 8, 10$, where the mean and median are both $6$.  When the value $10$ is replaced by $40$, the mean increases to $12$,
...
The median is a \index{robust statistic} **robust statistic** because unlike the mean it is  minimally affected by a few extreme observations at one end of a distribution.  For the same reason, the IQR is a robust measure of spread of a distribution, while the standard deviation is not. When $10$ is replaced by $40$ in the small dataset of 5 numbers, the standard deviation increases from $3.16$ to $15.81$, while the IQR ($8 - 4 = 4$) is unaffected..

The beeswarm plot shows that numerical summary statistics should be paired with visual displays whenever possible.

\index{box plot} **Box plots** provide a visual display of the robust measures of the location and spread along with the values that might influence the less robust mean and standard deviation, and is constructed from a five number summary. Figure @fig-trees-carbon-beeswarm-boxplot shows the beeswarm plot in Figure @fig-trees-carbon-beeswarm and a box plot below it.
...
With `Carbon_Storage_lb`, $Q_1 - 1.5 \times \textrm{IQR} = `r q1_r` - 1.5\times `r iqr_r` = `r lower_w`$ lbs. The lower whisker extends only to $0$ since there are no negative values of `Carbon_Storage_lb`. The upper whisker extends to $`r q1_r` + 1.5\times `r iqr_r` = `r upper_w`$ lbs.


Any observations beyond the whiskers is shown with dots and are labeled  outliers. An \index{outlier} **outlier** is a value that appears extreme relative to the rest of the data. There are many large outliers of `Carbon_Storage_lb`, as was suggested earlier by the beeswarm plot in Figure @fig-trees-carbon-beeswarm.

The beeswarm and box plots in Figure @fig-trees-carbon-beeswarm-boxplot provide different views of a distribution. The detail in the beeswarm shows how the observations of carbon storage values become less common with larger values.
...
A \index{histogram} **histogram** provides a balance between the fine detail of a beeswarm and the compact summary of a box plot. In a histogram, observations are grouped into bins and the height of a bar above each bin is the number of observations in the bin. Figure @fig-trees-carbon-histogram is a histogram of the `Carbon_Storage_lb` distribution.  The width of each bin is $1,000$ lbs, corresponding to the intervals $[0,1000), [1000, 2,000), \ldots, [15,000, 16,000)$.^[The default interval for the histogram in R is closed on the left, open on the right.]
...
```{r}
#| label: fig-trees-height-histogram
#| fig-cap: |
#|   Two plots showing the same variable, `Tree_Height` in ft. The upper image is a 
#|   box plot, the lower is a histogram. 
#| fig-subcap:
#|   - Box plot
#|   - Histogram
#| fig-alt: |
...
```{r five-num-summ-height}
#| label: tbl-trees-five-um-summ-height
#| tbl-cap: |
#|   The five number summary for `Tree Height` in Portland tree data.
#| tbl-pos: H
summ_stat_portland_trees<- portland_park_trees |> 
  summarize(
    Minimum = min(Tree_Height),
    `First Quartile (Q1)` = quantile(Tree_Height, probs = 0.25),
    Median = median(Tree_Height),
    Mean = mean(Tree_Height),
    `Third Quartile (Q3)` = quantile(Tree_Height, probs = 0.75),
