...
*add population median, IQR etc here?*
...
The units for these 4 values are lbs.
...
```{r }
#| include: false
terms_chp_1 <- c(terms_chp_1, c("data density", 
...
Contingency tables are characterized by the number of rows and columns corresponding to the levels of the two variables. The entries in regular font in @tbl-wvs-edu-health comprise a 3 $\times$ 5 table.   The \index{row totals} **row totals** and \index{column totals} ***column totals** in bold font in the last column and row have been added for convenience.  Collectively, these are referred to as the \index{marginal totals} **marginal totals**. Among the  `r n_cases`  respondents,  `r t_tot["Middle","Sum"]`  reported `Middle` education. The entry in the lower right corner is called the \index{grand total} **grand total**.
...
The relationship between 2 numerical variables can be explored with a \index{scatterplot} **scatterplot**, such as the one shown in @fig-crabs-width-weight-scatter.  In @fig-crabs-width-weight-scatter, `weight` (the vertical axis) is plotted against width (the horizontal axis). Scatterplots show a case-by-case view of the relationship between two numerical variables by showing individual data points.  For instance, the isolated point in the upper right corner of the plot represents the crab 33.5cm wide weighing 5200g.
